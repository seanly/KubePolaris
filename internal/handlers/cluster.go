...
/** genAI_main_start */
// GetNamespaces 获取集群的所有命名空间列表
func (h *ClusterHandler) GetNamespaces(c *gin.Context) {
	clusterID := c.Param("clusterID")

	logger.Info("获取命名空间列表", "cluster", clusterID)
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"code":    500,
			"message": fmt.Sprintf("创建K8s客户端失败: %v", err),
			"data":    nil,
		})
		return
	}
	...
	// 直接从 Kubernetes API 获取所有命名空间
	namespaceList, err := clientset.CoreV1().Namespaces().List(context.Background(), metav1.ListOptions{})
	if err != nil {
		logger.Error("获取命名空间列表失败", "cluster", cluster.Name, "error", err)
		c.JSON(http.StatusInternalServerError, gin.H{
			"code":    500,
