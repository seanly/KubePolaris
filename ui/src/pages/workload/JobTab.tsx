...
const JobTab: React.FC<JobTabProps> = ({ clusterId, onCountChange }) => {
  ...
  const loadNamespaces = useCallback(async () => {
    ...
    try {
      const response = await WorkloadService.getWorkloadNamespaces(clusterId, 'Job');
  ...
  const loadWorkloads = useCallback(async (page: number = currentPage, search: string = '') => {
    ...
    try {
      ...
      if (response.code === 200) {
        ...
        message.error(response.message || '获取Job列表失败');
      }
    } catch (error) {
      console.error('获取Job列表失败:', error);
      message.error('获取Job列表失败');
    } finally {
      setLoading(false);
    }
  ...
  const handleBatchDelete = async () => {
    ...
    try {
      ...
      if (failCount === 0) {
        message.success(`成功删除 ${successCount} 个Job`);
      } else {
  ...
  const columns: ColumnsType<WorkloadInfo> = [
    ...
    },
    {
      ...
      render: (record: WorkloadInfo) => (
        <span>
          {record.succeeded || 0} / {record.completions || 0}
        </span>
      ),
    },
    {
      ...
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          {selectedRowKeys.length > 0 && (
            <Button
              ...
              onClick={() => {
                Modal.confirm({
                  title: '批量删除确认',
                  content: `确定要删除选中的 ${selectedRowKeys.length} 个Job吗？`,
                  okText: '确定',
                  cancelText: '取消',
                  okType: 'danger',
        ...
        <Space>
          <Button
            type="primary"
            icon={<PlusOutlined />}
            onClick={() => navigate(`/clusters/${clusterId}/yaml/apply?type=Job`)}
          >
            创建Job
          </Button>
          <Button
