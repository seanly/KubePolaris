...
interface ArgoRolloutTabProps {
  clusterId: string;
  onCountChange?: (count: number) => void;
}

const ArgoRolloutTab: React.FC<ArgoRolloutTabProps> = ({ clusterId, onCountChange }) => {
  ...
  const loadNamespaces = useCallback(async () => {
    ...
    try {
      const response = await WorkloadService.getWorkloadNamespaces(clusterId, 'Rollout');
  ...
  const loadWorkloads = useCallback(async (page: number = currentPage, search: string = '') => {
    ...
    try {
      ...
      if (response.code === 200) {
        ...
        message.error(response.message || '获取Argo Rollout列表失败');
      }
    } catch (error) {
      console.error('获取Argo Rollout列表失败:', error);
      message.error('获取Argo Rollout列表失败');
    } finally {
      setLoading(false);
    }
  ...
  const handleBatchDelete = async () => {
    ...
    try {
      ...
      if (failCount === 0) {
        message.success(`成功删除 ${successCount} 个Argo Rollout`);
      } else {
              ...
              items: [
                {
                  ...
                  onClick: () => {
                    Modal.confirm({
                      title: '确认删除',
                      content: `确定要删除Argo Rollout ${record.name} 吗？`,
                      okText: '确定',
                      cancelText: '取消',
                      okType: 'danger',
    ...
    <div>
      ...
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          {selectedRowKeys.length > 0 && (
            <Button
              ...
              onClick={() => {
                Modal.confirm({
                  title: '批量删除确认',
                  content: `确定要删除选中的 ${selectedRowKeys.length} 个Argo Rollout吗？`,
                  okText: '确定',
                  cancelText: '取消',
                  okType: 'danger',
        ...
        <Space>
          <Button
            type="primary"
            icon={<PlusOutlined />}
            onClick={() => navigate(`/clusters/${clusterId}/yaml/apply?type=Rollout`)}
          >
            创建Argo Rollout
          </Button>
          <Button
      ...
      <Table
        ...
        pagination={{
          ...
          showTotal: (total) => `共 ${total} 个Argo Rollout`,
          onChange: (page, size) => {
            setCurrentPage(page);
            setPageSize(size || 20);
          },
      ...
      >
        {scaleWorkload && (
          <div>
            <p>Argo Rollout: <strong>{scaleWorkload.name}</strong></p>
