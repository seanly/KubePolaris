...
const DeploymentTab: React.FC<DeploymentTabProps> = ({ clusterId, onCountChange }) => {
  ...
  const loadNamespaces = useCallback(async () => {
    ...
    try {
      const response = await WorkloadService.getWorkloadNamespaces(clusterId, 'Deployment');
      ...
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          {selectedRowKeys.length > 0 && (
            <Button
              ...
              onClick={() => {
                Modal.confirm({
                  title: '批量删除确认',
                  content: `确定要删除选中的 ${selectedRowKeys.length} 个Deployment吗？`,
                  okText: '确定',
                  cancelText: '取消',
                  okType: 'danger',
        ...
        <Space>
          <Button
            type="primary"
            icon={<PlusOutlined />}
            onClick={() => navigate(`/clusters/${clusterId}/yaml/apply?type=Deployment`)}
          >
            创建Deployment
          </Button>
          <Button
