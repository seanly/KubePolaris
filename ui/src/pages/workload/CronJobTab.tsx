...
const CronJobTab: React.FC<CronJobTabProps> = ({ clusterId, onCountChange }) => {
  ...
  const loadNamespaces = useCallback(async () => {
    ...
    try {
      const response = await WorkloadService.getWorkloadNamespaces(clusterId, 'CronJob');
  ...
  const loadWorkloads = useCallback(async (page: number = currentPage, search: string = '') => {
    ...
    try {
      ...
      if (response.code === 200) {
        ...
        message.error(response.message || '获取CronJob列表失败');
      }
    } catch (error) {
  ...
  const handleBatchDelete = async () => {
    ...
    try {
      ...
      if (failCount === 0) {
        message.success(`成功删除 ${successCount} 个CronJob`);
      } else {
  ...
  const columns: ColumnsType<WorkloadInfo> = [
    ...
    },
    {
      ...
      render: (suspend: boolean) => (
        <Tag color={suspend ? 'warning' : 'success'}>
          {suspend ? '是' : '否'}
        </Tag>
      ),
    },
    ...
    <div>
      ...
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          {selectedRowKeys.length > 0 && (
            <Button
              ...
              onClick={() => {
                Modal.confirm({
                  title: '批量删除确认',
                  content: `确定要删除选中的 ${selectedRowKeys.length} 个CronJob吗？`,
                  okText: '确定',
                  cancelText: '取消',
                  okType: 'danger',
        ...
        <Space>
          <Button
            type="primary"
            icon={<PlusOutlined />}
            onClick={() => navigate(`/clusters/${clusterId}/yaml/apply?type=CronJob`)}
          >
            创建CronJob
          </Button>
          <Button
      ...
      <Table
        ...
        pagination={{
          ...
          showTotal: (total) => `共 ${total} 个CronJob`,
          onChange: (page, size) => {
            setCurrentPage(page);
            setPageSize(size || 20);
          },
